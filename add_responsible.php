...
$query = $mysqli->prepare("SELECT * FROM dne_responsibles WHERE id = ? AND id_project = ?");
		...
		<form method="post" action="" enctype="multipart/form-data" class="form-inline">
		    ...
		    <div class="container">
				...
				if($id == 0) { ?>
					<div class="row title">
						<div class="col-md-12">
							הוסף אחראי לפרוייקט <span style="color:#5bbd8d;"><?=@$project->name?></span>
						</div>
					</div>
				<?php }
				else if($id > 0) { ?>
				   <div class="row title">
						<div class="col-md-12">
							<span style="color:#5bbd8d;"><?=@$responsible->name?></span>
						</div>
					</div>
				<?php } ?>		
					...
					<div class="col-md-12">
						...
						<select id="suppliers" class="form-control" style="width:200px;">
							...
							<?php foreach ($suppliers as $item) { ?>
								<option value="<?=@$item->id?>" <?php if($item->id == @$responsible->id_projects_suppliers) echo 'selected';?>><?=@$item->name_he?></option>
							<?php } ?>
							</optgroup>
							<optgroup label="משרד">
							...
							<optgroup label="יזם">
								<option value="<?=@$entrepreneur->id?>" <?php if($entrepreneur->id == @$responsible->id_projects_suppliers) echo 'selected';?>><?=@$entrepreneur->name_he?></option>
							</optgroup>
						</select>
					</div>
				</div>			
				...
				<div class="row" style="margin-top:20px;direction:rtl;">
					<div class="col-md-11">	
						<strong>שם:</strong>
						<br/>	
						<input type="text" class="form-control" style="width:250px;" name="name" id="name" placeholder="*שם" value="<?=@$responsible->name?>" />	
					</div>
				</div>
				...
				<div class="row" style="margin-top:20px;direction:rtl;">
					<div class="col-md-11">
						...
						&nbsp;						
						<input type="color" class="form-control" style="width:150px;" name="color" id="color" placeholder="צבע גופן" value="<?=@$responsible->color?>" />
					</div>
				</div>
				...
				<div class="row" style="margin-top:20px;direction:rtl;">
					<div class="col-md-11">
						...
						&nbsp;						
						<input type="color" class="form-control" style="width:150px;" name="bgcolor" id="bgcolor" placeholder="צבע רקע" value="<?php if($id == 0) echo '#ffffff';else echo @$responsible->bgcolor;?>" />
					</div>
				</div>				
				...
				<div class="row" style="margin-top:20px;direction:rtl;">
					<div class="col-md-11">	
						<strong>דוא''ל:</strong>
						<br/>	
						<input type="email" class="form-control" style="width:250px;" name="email" id="email" placeholder="דוא''ל" value="<?=@$responsible->email?>" />	
					</div>
				</div>
