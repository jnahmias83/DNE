...
$query->bind_param("ii",$_SESSION['id_projects_suppliers'],$project_id);
$query->execute();
$query->store_result();
        ...
        <form method="post" action="" class="form-inline">
			...
			<div class="container">
			    <div class="row title">	
					<div class="col-md-12">
						<a style="text-decoration:underline;" href="project_home.php?id=<?=@$project_id?>">
							<?=@$project->name_he.'<br/>'.$supplier->name_he.'<br/>דו\'ח משימות<br/>'.substr(date('Y-m-d'),8,2).'/'.substr(date('Y-m-d'),5,2).'/'.substr(date('Y-m-d'),0,4)?>
						</a>
					</div>					
			    </div>
				...
				<?php if($all_meetings_num_rows > 0) { ?>		
					<div class="row" style="font-size:12px;text-align:center;direction:rtl;">
						<div align="center" class="col-md-12 mx-1" style="overflow-x:scroll;">
							<table border="1">							
								...
								<?php
								$sql_chapters = "SELECT * FROM dne_chapters WHERE id_project = ? ORDER BY id_display";
								...
								foreach($chapters as $item) {	
									$chapter_id = $item->id;			
									$query = $mysqli->prepare($sql.' AND m.id_chapter ='.$chapter_id.' ORDER BY t.id_display,m.subject,m.id_area,m.destination_date DESC');
									$query->execute(); 
									$query->store_result();
									...
									?>
										...
										foreach($meetings as $item) {
											...
											<tr>
												...
												<td style="text-align:center;<?=@$update_cell_bg_color?>;"><a onclick="location.href='add_meeting.php?&project_id=<?=@$project_id?>&id=<?=@$meeting_id?>'" title="עדכן" style="text-decoration:underline;cursor:pointer;color:<?=@$color_num?>"><?=@$count?></a></td>										
					...
					<div class="row" style="text-align:center;">
						<div class="col-md-12">
						   <input type="button" value="דו'ח סטטוס פרוייקט" class="btn marginTop20 mb-2" style="font-size:14px;width:140px;" onclick="toFilteredMeetingsReport();" />
						</div>
					</div>
...
function toFilteredMeetingsReport() {
	var form_data = new FormData();	
		...
		$.ajax({
			...
			contentType: false,			
			success: function(data){
			    window.open('results_filters_meetings_by_responsible_report.php?project_id='+$('#project_id').val(),'_blank');
			},
		});		
}
</script>
