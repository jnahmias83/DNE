...
$title = $project->name_he.'<br/>דו\'ח סטטוס פרוייקט <br/>'.substr(date('Y-m-d'),8,2).'/'.substr(date('Y-m-d'),5,2).'/'.substr(date('Y-m-d'),0,4);
...
foreach($chapters as $item) {	
	...
	$query = $mysqli->prepare("SELECT m.id AS id,m.subject AS subject,m.id_rdv AS id_rdv,m.id_area AS id_area,m.description AS description,m.id_task AS id_task,m.id_responsible AS id_responsible,
							  ...
							  c.name AS chapter_name,m.image1 AS image1,m.image2 AS image2
	                          FROM dne_meetings m
							  LEFT JOIN dne_chapters c ON m.id_chapter = c.id
							  LEFT JOIN dne_tasks t ON m.id_task = t.id
							  WHERE m.id_project = ? AND m.id_chapter = ? AND m.is_appears = ? AND m.is_pdf_appears = ?
							  ORDER BY t.id_display,m.subject,m.id_area,m.destination_date DESC");
	$query->bind_param("iiii",$project_id,$chapter_id,$is_appears,$is_pdf_appears);
	$query->execute(); 
	$query->store_result();
...
foreach($chapters as $item) {	
	...
	$query = $mysqli->prepare("SELECT m.id AS id,m.subject AS subject,m.id_rdv AS id_rdv,m.id_area AS id_area,m.description AS description,m.id_task AS id_task,m.id_responsible AS id_responsible,
							  ...
							  LEFT JOIN dne_chapters c ON m.id_chapter = c.id
							  LEFT JOIN dne_tasks t ON m.id_task = t.id
							  WHERE m.id_project = ? AND m.id_chapter = ? AND m.is_appears = ? AND m.is_pdf_appears = ? AND (image1 <> '' OR image2 <> '')
							  ORDER BY t.id_display,m.subject,m.id_area,m.destination_date DESC");
	$query->bind_param("iiii",$project_id,$chapter_id,$is_appears,$is_pdf_appears);
	$query->execute(); 
	$query->store_result();
