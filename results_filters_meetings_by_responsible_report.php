...
$title = $project->name_he.'<br/>'.$supplier->name_he.'<br/>דו\'ח משימות<br/>'.substr(date('Y-m-d'),8,2).'/'.substr(date('Y-m-d'),5,2).'/'.substr(date('Y-m-d'),0,4);
...
foreach($chapters as $item) {	
	...
	$is_pdf_appears = 1;
	$query = $mysqli->prepare($sql.' AND m.id_chapter ='.$chapter_id.' AND m.is_pdf_appears = ? ORDER BY t.id_display,m.subject,m.id_area,m.destination_date DESC');
...
foreach($chapters as $item) {	
	...
	$is_pdf_appears = 1;
	$query = $mysqli->prepare($sql.' AND m.id_chapter ='.$chapter_id.' AND m.is_pdf_appears = ? AND m.image1 <> 1 AND (m.image1 <> \'\' OR m.image2 <> \'\') ORDER BY t.id_display,m.subject,m.id_area,m.destination_date DESC');
	$query->bind_param("i",$is_pdf_appears);
	$query->execute(); 
	$query->store_result();
	$meetings_num_rows = $query->num_rows;
